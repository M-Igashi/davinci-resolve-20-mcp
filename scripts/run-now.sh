...
# Use new modular server by default, fall back to legacy
if [ -f "$ROOT_DIR/src/server.py" ]; then
    SERVER_PATH="$ROOT_DIR/src/server.py"
    echo -e "${GREEN}Using modular server (v2.0.0)${NC}"
else
    SERVER_PATH="$ROOT_DIR/src/resolve_mcp_server.py"
    echo -e "${YELLOW}Using legacy server${NC}"
fi
...
if pgrep -i "DaVinci Resolve" > /dev/null 2>&1 || pgrep -i "Resolve" > /dev/null 2>&1; then
...
if [ ! -f "$VENV_DIR/bin/mcp" ]; then
    ...
    "$VENV_DIR/bin/pip" install -q "mcp[cli]"
...
echo -e "${YELLOW}Server: $SERVER_PATH${NC}"
echo ""
